...
import { cosine } from "../utils/vectorMath.js";
...
export class MemoryVectorStore {
  ...
  async add(params) {
    ...
    if (embedding && embedding.length !== this.embeddingDim) {
      throw new Error(`embedding dimension ${embedding.length} does not match collection embedding dimension ${this.embeddingDim}`);
    }
    if (this.rows.has(id)) {
  ...
  async update(params) {
    ...
    this.rows.set(id, {
      ...
      embedding: embedding ?? (document ? await this.embeddings.embed(document) : oldRow.embedding),
  ...
  async query(params) {
    ...
    if (queryEmbedding && queryEmbedding.length !== this.embeddingDim) {
      throw new Error(`queryEmbedding dimension ${queryEmbedding.length} does not match collection embedding dimension ${this.embeddingDim}`);
    }
    const searchEmbedding = queryEmbedding ?? (await this.embeddings.embed(queryText));
    return Array.from(this.rows.values()).map(r => ({
      ...r,
      similarity: cosine(searchEmbedding, r.embedding)
    })).filter(predicate ?? (() => true)).sort((a, b) => b.similarity - a.similarity).slice(0, nResults);
  }
}
